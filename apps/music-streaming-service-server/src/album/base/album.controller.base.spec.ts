...
const service = {
  ...
  albums: () => FIND_MANY_RESULT,
  album: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Album", () => {
  ...
  test("GET /albums/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/albums"}/${nonExistingId}`)
  ...
  test("GET /albums/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/albums"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /albums existing resource", async () => {
